...
from api.chatbot_memory import ConversationMemory

logger = logging.getLogger(__name__)


# ============================================================================
# SYSTEM PROMPT â€” Domain-specific context for the drug response project
# ============================================================================
...
# Embedded system prompt â€” used as fallback if prompts/system_prompt.txt is missing
SYSTEM_PROMPT = """You are an AI assistant for the Drug Response Prediction and Recommendation Platform â€” a graduation project in bioinformatics. Your role is to help users understand the system's predictions, methodology, and scientific concepts.
...
2. **Feature Selection**: Exactly 9 features are selected â€” AUC, Z_SCORE, and 7 encoded columns (TARGET_ENC, TARGET_PATHWAY_ENC, TCGA_DESC_ENC, GDSC_TISSUE_DESCRIPTOR_2_ENC, CANCER_TYPE_MATCHING_TCGA_LABEL_ENC, SITE_ENC, GDSC_TISSUE_DESCRIPTOR_1_ENC). Note: CELL_LINE_NAME_ENC and DRUG_NAME_ENC are NOT in the final features â€” the model uses biological/pharmacological characteristics instead of raw identifiers.
3. **Standard Scaling**: The 9 selected features are normalized to zero mean and unit variance using a saved StandardScaler (x_scaled = (x - Î¼) / Ïƒ).

### Prediction Pipeline:
- Input â†’ Preprocessing (load from disk, never fit at runtime) â†’ Model prediction â†’ IC50 output
...
- Be scientific but accessible â€” explain complex concepts clearly
...
- Be concise but thorough â€” don't oversimplify, but don't overwhelm
...
class ChatbotService:
    ...
    def _call_gemini(self, message: str, conversation_id: str) -> str:
        ...
        if response.text:
            ...
            # Empty response â€” check for safety filter blocks
            try:
...
# ============================================================================
# SINGLETON INSTANCE â€” Matches the pattern used by MLService
# ============================================================================
